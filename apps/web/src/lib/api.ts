...
type CacheKey = "dashboard" | "games" | "alerts" | "programs" | "event" | "events" | "results" | "resultsAll";
...
export async function apiGet<T>(path: string, cacheKey?: CacheKey): Promise<T> {
  ...
  try {
    ...
    if (cacheKey && storageKey && typeof window !== "undefined") {
      preserveDashboardFollowsForMigration(cacheKey, window.localStorage.getItem(storageKey), data);
      window.localStorage.setItem(storageKey, JSON.stringify({ data, savedAt: new Date().toISOString() }));
    }
    return data;
  } catch (error) {
...
export const CourtWatchApi = {
  ...
  dashboard: (eventId?: number | null) => apiGet<DashboardResponse>(withEvent("/api/dashboard", eventId), "dashboard"),
  event: (eventId?: number | null) => apiGet<TournamentEvent>(withEvent("/api/events/current", eventId), "event"),
  programs: (eventId?: number | null) => apiGet<ProgramSummary[]>(withEvent("/api/programs", eventId), "programs"),
  games: (query = "", eventId?: number | null) => apiGet<Game[]>(withEvent(`/api/games${query}`, eventId), "games"),
  results: (scope: "watched" | "all" = "watched", eventId?: number | null) =>
    apiGet<DivisionResultGroup[]>(withEvent(`/api/results?scope=${scope}`, eventId), scope === "all" ? "resultsAll" : "results"),
  alerts: (eventId?: number | null) => apiGet<GameChangeEvent[]>(withEvent("/api/alerts", eventId), "alerts"),
  teams: (search = "", eventId?: number | null) => apiGet<Team[]>(withEvent(`/api/teams${search ? `?search=${encodeURIComponent(search)}` : ""}`, eventId)),
...
function cacheStorageKey(cacheKey: CacheKey, path: string): string {
  return `courtwatch-aau:${cacheKey}:${path}`;
}
