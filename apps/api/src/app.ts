...
export function createApp(store: CourtWatchStore, prismaClient: PrismaClient | null = null) {
  ...
  app.get("/api/dashboard", async (req, res, next) => {
    try {
      res.json(await store.dashboard(requestClientId(req), requestExposureEventId(req)));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/programs", async (req, res, next) => {
    try {
      res.json((await store.dashboard(requestClientId(req), requestExposureEventId(req))).programs);
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/programs/:programId", async (req, res, next) => {
    try {
      const program = await store.program(req.params.programId, requestClientId(req), requestExposureEventId(req));
      if (!program) {
  ...
  app.get("/api/teams", async (req, res, next) => {
    try {
      res.json(await store.teams(typeof req.query.search === "string" ? req.query.search : undefined, requestClientId(req), requestExposureEventId(req)));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/results", async (req, res, next) => {
    try {
      ...
      res.json(await store.results(requestClientId(req), scope, requestExposureEventId(req)));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.get("/api/alerts", async (req, res, next) => {
    try {
      res.json(await store.alerts(requestClientId(req), requestExposureEventId(req)));
    } catch (error) {
      next(error);
    }
  });
...
function requestExposureEventId(req: express.Request): number | null {
  const queryValue = stringQuery(req.query.eventId) ?? stringQuery(req.query.exposureEventId);
  const body = req.body as { eventId?: unknown; exposureEventId?: unknown } | undefined;
  const bodyValue = body?.eventId ?? body?.exposureEventId;
  const raw = queryValue ?? (bodyValue === undefined || bodyValue === null ? undefined : String(bodyValue));
  if (!raw) return null;
