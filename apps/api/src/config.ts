...
import { DEFAULT_MAJOR_TOURNAMENT_SOURCES, DEFAULT_TOURNAMENT_TIMEZONE } from "@courtwatch/core";
...
const fallbackTournament: TournamentSource = {
  ...
  sourceUrl: "https://basketball.exposureevents.com/255539/2026-reno-memorial-day-tournament",
...
const TournamentSourceSchema = z
  .object({
    ...
    status: z.enum(["upcoming", "active", "completed", "unavailable", "cancelled"]).default("upcoming")
  })
  .transform((value): TournamentSource => {
    ...
    return {
      ...
      slug: value.slug,
      sourceUrl: value.sourceUrl ?? value.officialUrl ?? `https://basketball.exposureevents.com/${value.exposureEventId}/${value.slug}`,
      name: value.name,
...
export function majorTournamentSources(): MajorTournamentSource[] {
  ...
  if (!config.MAJOR_TOURNAMENT_SOURCES?.trim()) return DEFAULT_MAJOR_TOURNAMENT_SOURCES;
  try {
    ...
    throw new Error(`MAJOR_TOURNAMENT_SOURCES must be valid JSON: ${error instanceof Error ? error.message : "unknown parse error"}`);
  }
}
