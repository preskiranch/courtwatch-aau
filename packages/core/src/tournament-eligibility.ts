...
export function deriveTournamentStatus(event: Pick<TournamentEvent, "startDate" | "endDate" | "status">, todayKey = tournamentTodayKey()): TournamentEventStatus {
  if (event.status === "cancelled" || event.status === "unavailable") return event.status;
...
export function eligibleTournamentEvents(events: TournamentEvent[], options: TournamentDropdownEligibilityOptions = {}): TournamentEvent[] {
  ...
  for (const event of events) {
    ...
    seen.add(key);
    if (isTournamentDropdownEligible(event, options)) deduped.push({ ...event, status: deriveTournamentStatus(event, options.todayKey ?? tournamentTodayKey(options.now)) });
  }
  return deduped.sort((left, right) => left.startDate.localeCompare(right.startDate) || left.name.localeCompare(right.name));
}

export function hasRecentSuccessfulTournamentData(
  event: Pick<TournamentEvent, "lastSyncedAt" | "lastCheckedAt">,
  options: Pick<TournamentDropdownEligibilityOptions, "cacheHours" | "now"> = {}
): boolean {
  const reference = event.lastSyncedAt ?? event.lastCheckedAt;
  if (!reference) return false;
  const checkedAt = Date.parse(reference);
  if (!Number.isFinite(checkedAt)) return false;
  const cacheMs = (options.cacheHours ?? DEFAULT_DROPDOWN_CACHE_HOURS) * 60 * 60 * 1000;
  return (options.now ?? new Date()).getTime() - checkedAt <= cacheMs;
}

export function tournamentDedupeKey(event: TournamentEvent): string {
  if (event.externalProvider && event.externalId) return `${event.externalProvider}:${event.externalId}`.toLowerCase();
  return [event.name, event.startDate, event.city, event.state, event.organizer].map((part) => normalizeKey(part ?? "")).join("|");
}
