...
export class PublicExposurePageClient {
  ...
  async fetchTeams(eventId: number, eventSlug = "2026-reno-memorial-day-tournament", timezone = DEFAULT_TOURNAMENT_TIMEZONE): Promise<PublicExposureTeamResult> {
    ...
    const response = await this.fetchImpl(url, {
      headers: {
        "User-Agent": "CourtWatchAAU/0.1 (+independent companion tracker; respectful cache-backed polling)"
      }
    });
    if (!response.ok) {
    ...
    for (const division of divisions) {
      ...
      for (const group of rawGroups) {
        ...
        for (const rawGame of group.Games) {
          const mapped = mapPublicExposureGame(rawGame, eventId, bracketsByDivision.get(String(rawGame.DivisionId ?? division.Id)) ?? [], eventSlug, this.baseUrl, timezone);
...
function mapPublicExposureGame(raw: PublicExposureGameRaw, eventId: number, bracketLinks: PublicExposureBracketLink[], eventSlug: string, baseUrl: string, timezone: string): Game | null {
  ...
  return {
    ...
    eventId: `event-${eventId}`,
    divisionId: divisionExposureId ? `division-${eventId}-${divisionExposureId}` : null,
    exposureGameId: id,
    ...
    streamingUrl: raw.StreamingId ? `https://www.ballertv.com/events/${eventSlug}/games/${raw.StreamingId}` : null,
