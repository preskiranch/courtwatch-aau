...
import { ExposureEventsTournamentProvider, PublicHtmlTournamentProvider, TournamentDiscoveryService, type TournamentProvider } from "./tournament-discovery.js";
...
describe("TournamentDiscoveryService", () => {
  ...
  it("includes non-Exposure public HTML tournaments when a public team-list page is reachable", async () => {
    const fetchImpl = vi.fn(async (input: string | URL | Request) => {
      ...
      if (url.endsWith("/events")) {
        return htmlResponse('<a href="/metro-hoop-classic">Metro Hoop Classic</a>');
      }
    ...
    expect(result.candidates[0]?.teams.teams.map((team) => team.name)).toEqual(["Phoenix Elite", "Desert Select"]);
  });
  ...
  it("dedupes tournaments discovered by different public providers", async () => {
    ...
    const publicHtmlProvider: TournamentProvider = {
      ...
      supportsPublicTeamLists: true,
      discoverEvents: async () => [{ ...event, id: "event-1800000000", exposureEventId: 1800000000, externalProvider: "public_html", externalId: "same-classic-public" }],
      fetchRegisteredTeams: async () => ({ divisions: [], teams: [] })
    };

    const result = await new TournamentDiscoveryService([exposureProvider, publicHtmlProvider]).discover(
      [
    ...
    expect(result.failures).toEqual([]);
    expect(result.candidates.map((candidate) => candidate.event.externalProvider)).toEqual(["exposure_events"]);
  });
