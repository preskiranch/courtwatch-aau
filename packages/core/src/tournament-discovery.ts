...
export type TournamentProviderName = "exposure_events" | "public_html" | "aau_event_finder";
...
export class TournamentDiscoveryService {
  ...
  constructor(providers: TournamentProvider[] = [new ExposureEventsTournamentProvider(), new PublicHtmlTournamentProvider(), new AauEventFinderTournamentProvider()]) {
...
export class PublicHtmlTournamentProvider implements TournamentProvider {
  ...
  async discoverEvents(source: MajorTournamentSource, window: TournamentDiscoveryWindow): Promise<DiscoveredTournamentEvent[]> {
    const eventUrls = new Set<string>();
    for (const eventUrl of source.eventUrls ?? []) eventUrls.add(normalizePublicUrl(eventUrl));

    if (source.url && (source.eventLinkPatterns?.length ?? 0) > 0) {
      const sourceUrl = normalizePublicUrl(source.url);
      const html = await this.fetchText(sourceUrl);
      for (const eventUrl of parsePublicHtmlEventLinks(html, sourceUrl, source.eventLinkPatterns ?? [])) eventUrls.add(eventUrl);
    }
  ...
  async fetchRegisteredTeams(event: DiscoveredTournamentEvent): Promise<PublicExposureTeamResult> {
    ...
    const divisions = new Map<string, PublicExposureTeamResult["divisions"][number]>();
    ...
    for (const teamListUrl of teamListUrls.slice(0, maxTeamListPages)) {
      ...
      for (const division of parsed.divisions) divisions.set(division.id, division);
      for (const team of parsed.teams) teams.set(team.id, team);
      await sleep(Number(process.env.TOURNAMENT_DISCOVERY_REQUEST_DELAY_MS ?? 125));
    }

    if (!fetchedAnyTeamListPage) throw new Error("No reachable public team-list page was found for this event.");
    return { divisions: Array.from(divisions.values()), teams: Array.from(teams.values()) };
  }
  ...
  private async fetchEventDetails(eventUrl: string, source: MajorTournamentSource): Promise<DiscoveredTournamentEvent> {
    ...
    const dateRange = dateRangeFromJsonLd(jsonLdEvent) ?? parseDateRange(`${title} ${description} ${bodyText.slice(0, 1000)}`);
    if (!dateRange) throw new Error(`Could not parse public event date range from ${normalizedEventUrl}`);

    const name = cleanText(jsonLdEvent.name ?? stripTitleSuffix(title, dateRange.raw) ?? title);
    if (!name) throw new Error(`Could not parse public event name from ${normalizedEventUrl}`);
    const sourceText = `${source.name} ${source.organizerName ?? ""} ${source.sanctioningTags?.join(" ") ?? ""} ${title} ${description} ${bodyText.slice(0, 1200)}`;
    if (!/\bbasketball\b/i.test(sourceText)) throw new Error(`Public HTML event does not appear to be basketball: ${normalizedEventUrl}`);
    ...
    const syntheticEventId = publicSyntheticEventId(this.providerName, externalId);
    const status = bodyText.match(/\bcancelled\b/i) ? "cancelled" : deriveTournamentStatus({ startDate: dateRange.startDate, endDate: dateRange.endDate, status: "upcoming" });
    ...
    return {
      ...
      sport: "basketball",
      sanctioningTags: dedupeStrings([...(source.sanctioningTags ?? []), ...parseSanctioningTags(bodyText), "Public Source"]),
      gender: parseGender(bodyText),
      ageOrGradeDivisions: parseAgeOrGradeDivisions(bodyText),
      venueName: null,
      city,
      ...
      startDate: dateRange.startDate,
      endDate: dateRange.endDate,
      location: location || [city, state].filter(Boolean).join(", ") || "Location TBD",
  ...
  private async fetchText(url: string): Promise<string> {
    ...
    if (!response.ok) throw new Error(`Public HTML page request failed with ${response.status}`);
    return response.text();
  }
}
...
function tournamentDiscoveryDedupeKeys(event: DiscoveredTournamentEvent): string[] {
  return [
    tournamentDedupeKey(event),
    ["fallback", event.name, event.startDate, event.city, event.state, event.organizer].map((part) => normalizeName(part ?? "")).join("|")
  ];
}
...
function parsePublicHtmlEventLinks(html: string, sourceUrl: string, patterns: string[]): string[] {
  ...
  $("a[href]").each((_, element) => {
    ...
    if (regexes.some((regex) => regex.test(href) || regex.test(absoluteUrl) || regex.test(text))) urls.add(absoluteUrl);
  });
  return Array.from(urls);
}

function publicTeamListUrls(eventUrl: string, eventHtml: string, source?: MajorTournamentSource): string[] {
  const urls = new Set<string>();
  const event = new URL(eventUrl);
  const slug = slugFromPublicUrlOrName(eventUrl, event.pathname.split("/").filter(Boolean).at(-1) ?? "event");
  ...
  const patterns = (source?.teamListLinkPatterns?.length ? source.teamListLinkPatterns : ["teams?", "registered\\s+teams?", "participating\\s+teams?", "team\\s+list", "clubs?"]).map(
  ...
  $("a[href]").each((_, element) => {
    ...
    if (patterns.some((regex) => regex.test(href) || regex.test(absoluteUrl) || regex.test(text))) urls.add(absoluteUrl);
  });
...
function looksLikeTeamListPage(url: string, html: string, source?: MajorTournamentSource): boolean {
  if ((source?.teamSelectors?.length ?? 0) > 0) return true;
  const $ = cheerio.load(html);
  const titleText = cleanText(`${$("title").text()} ${$("h1,h2,h3").map((_, element) => $(element).text()).get().join(" ")}`);
  return /\b(teams?|registered teams?|participating teams?|team list|clubs?)\b/i.test(`${url} ${titleText}`);
}

function parsePublicHtmlTeams(html: string, event: DiscoveredTournamentEvent, sourceUrl: string, source?: MajorTournamentSource): PublicExposureTeamResult {
  const $ = cheerio.load(html);
  const divisions = new Map<string, PublicExposureTeamResult["divisions"][number]>();
  const teams = new Map<string, PublicExposureTeamResult["teams"][number]>();
  const selectors = source?.teamSelectors?.length ? source.teamSelectors : ["[data-team-name]"];

  for (const selector of selectors) {
    $(selector).each((_, element) => {
      const node = $(element);
      const teamName = cleanTeamName(node.attr("data-team-name") || node.text());
      if (!teamName) return;
      const divisionName = cleanText(node.attr("data-division") || closestDivisionName(node) || "Unknown Division");
      addPublicHtmlTeam({ event, sourceUrl, teamName, divisionName, divisions, teams });
    });
  }
...
function addPublicHtmlTeam({
  ...
  const teamKey = stableKey(`${event.externalProvider}|${event.externalId}|${divisionName}|${teamName}`);
...
function parseTeamTables(
  ...
  $("table").each((_, table) => {
    const headers = $(table)
      ...
      .get();
    const teamColumn = headers.findIndex((header) => header === "team" || header === "team name");
    if (teamColumn < 0) return;
    const divisionName = cleanText($(table).prevAll("h1,h2,h3,h4").first().text()) || "Unknown Division";
    $(table)
      .find("tr")
      .each((__, row) => {
...
function parseTeamTextSections(
  ...
  for (const line of lines) {
    if (/^(teams?|registered teams?|participating teams?|team list)$/i.test(line)) {
      inTeams = true;
      continue;
    }
    if (!inTeams) continue;
    if (/^(schedule|bracket|champions?|contact|registration|location|documents?|rules?|standings?)$/i.test(line)) break;
...
function closestDivisionName(node: cheerio.Cheerio<AnyNode>): string | null {
  return cleanText(node.closest("[data-division]").attr("data-division") || node.prevAll("h1,h2,h3,h4").first().text()) || null;
}
...
function parseJsonLdEvent($: cheerio.CheerioAPI): JsonLdEvent {
  for (const script of $("script[type='application/ld+json']").toArray()) {
    ...
    for (const item of flattenJsonLd(parsed)) {
      const typeValue = Array.isArray(item["@type"]) ? item["@type"].join(" ") : String(item["@type"] ?? "");
...
function dateRangeFromJsonLd(event: JsonLdEvent): { startDate: string; endDate: string; raw: string } | null {
  const startDate = dateKeyFromUnknown(event.startDate);
  if (!startDate) return null;
  const endDate = dateKeyFromUnknown(event.endDate) ?? startDate;
  return { startDate, endDate, raw: [event.startDate, event.endDate].filter(Boolean).join(" - ") || startDate };
}
...
function jsonLdLocation(value: unknown): string {
  ...
  if (isRecord(address)) {
    return [address.addressLocality, address.addressRegion].filter((part): part is string => typeof part === "string" && Boolean(part.trim())).join(", ");
  }
  return [value.name, value.addressLocality, value.addressRegion].filter((part): part is string => typeof part === "string" && Boolean(part.trim())).join(", ");
}
...
function cleanTeamName(value: string): string | null {
  ...
  if (/^(team|teams|schedule|standings|bracket|pool|division|select|loading|register|notifications?|share|copy|score|w|l|pd|ps|pa)$/i.test(text)) return null;
...
function isLikelyDivisionName(value: string): boolean {
  return /\b(boys?|girls?|grade|u|varsity|division|gold|silver|bronze|platinum|pool)\b/i.test(value) && value.length <= 80;
}
...
function slugFromPublicUrlOrName(value: string, name: string): string {
  const pathSlug = new URL(value).pathname
    .split("/")
    .filter(Boolean)
    .at(-1);
...
function publicSyntheticEventId(provider: string, externalId: string): number {
  return 1_000_000_000 + (stableHash(`${provider}|${externalId}`) % 900_000_000);
}
...
async function assertRobotsAllowed(url: string, fetchImpl: typeof fetch) {
  ...
  if (!isRobotsPathAllowed(parsed.pathname, rules)) throw new Error(`robots.txt disallows public discovery for ${parsed.origin}${parsed.pathname}`);
}

function robotsRulesForOrigin(origin: string, fetchImpl: typeof fetch): Promise<RobotsRules> {
...
function parseRobotsTxt(text: string): RobotsRules {
  ...
  const matchingGroups = groups.filter((group) => group.agents.some(robotsAgentMatches));
  const nonWildcardGroups = matchingGroups.filter((group) => !group.agents.includes("*"));
  const selectedGroups = nonWildcardGroups.length > 0 ? nonWildcardGroups : matchingGroups;
...
function isRobotsPathAllowed(pathname: string, rules: RobotsRules): boolean {
  const matchingDisallows = rules.disallow.filter((rule) => robotsRuleMatches(pathname, rule));
  if (matchingDisallows.length === 0) return true;
  const matchingAllows = rules.allow.filter((rule) => robotsRuleMatches(pathname, rule));
